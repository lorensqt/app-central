...
<body class="h-full flex flex-col justify-between relative bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 overflow-x-hidden min-h-screen transition-colors duration-300">
    ...
    <div class="flex-grow flex items-center justify-center p-4 sm:p-6 lg:p-8 z-10 relative">
        <div class="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start opacity-0 translate-y-8 animate-fade-in-up">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-150/80 dark:border-slate-800/80 p-4 sm:p-5 lg:p-6 shadow-[0_8px_30px_rgba(15,23,42,0.015)] hover:shadow-[0_20px_50px_rgba(168,85,247,0.05)] hover:border-purple-200/80 dark:hover:border-purple-800/60 transition-all duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 group relative bg-gradient-to-r from-white to-slate-50/50 dark:from-slate-900 dark:to-slate-900/50 hover:to-purple-50/20 dark:hover:to-purple-950/20">
                        ...
                        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
                            ...
                            <div class="space-y-0.5 min-w-0">
                                ...
                                <h4 class="text-sm sm:text-base lg:text-lg font-extrabold text-slate-800 dark:text-slate-200 leading-snug break-words tracking-tight mt-1">
                    ...
                    <a href="https://www.google.com/maps/search/?api=1&query={{ urlencode($event->location) }}" target="_blank" rel="noopener noreferrer" 
                        ...
                        <div class="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                            ...
                            <div class="space-y-0.5 min-w-0">
                                ...
                                <h4 class="text-sm sm:text-base lg:text-lg font-extrabold text-slate-800 dark:text-slate-200 leading-snug break-words tracking-tight mt-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                    {{ $event->location }}
                                </h4>
                            </div>
                        </div>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-150/80 dark:border-slate-800/80 p-4 sm:p-5 lg:p-6 shadow-[0_8px_30px_rgba(15,23,42,0.015)] hover:shadow-[0_20px_50px_rgba(var(--hover-shadow-rgb),0.05)] hover:border-{{ $themeColor }}-200/80 dark:hover:border-{{ $themeColor }}-800/60 transition-all duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 group min-w-0 relative overflow-hidden bg-gradient-to-r from-white to-slate-50/50 dark:from-slate-900 dark:to-slate-900/50 hover:to-{{ $themeColor }}-50/20 dark:hover:to-{{ $themeColor }}-950/20"
                        ...
                        <div class="flex items-center gap-3 sm:gap-4 min-w-0">
                            ...
                            <div class="space-y-0.5 min-w-0">
                                <div class="text-[9px] sm:text-[10px] font-bold text-{{ $themeColor }}-500 dark:text-{{ $themeColor }}-400 uppercase tracking-widest leading-none">Seat Registration</div>
                                <h4 class="text-sm sm:text-base lg:text-lg font-extrabold text-slate-800 dark:text-slate-200 leading-snug break-words tracking-tight mt-1">
                                    {{ $titleText }}
                                </h4>
                            </div>
                        ...
                        <div class="w-full sm:w-48 shrink-0">
                            @if($hasLimit && !$isFullyBooked)
                                <div class="flex justify-between items-center text-[9px] sm:text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1">
                                    <span>Occupancy</span>
                                ...
                                <div class="text-[9px] sm:text-[10px] text-right text-slate-400 dark:text-slate-500 mt-1 font-semibold">
                                    {{ $subtext }}
                                </div>
            ...
            <div class="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-[0_12px_40px_rgba(15,23,42,0.03)] hover:shadow-[0_16px_48px_rgba(15,23,42,0.06)] transition-all duration-500">
                @if($isFullyBooked)
                        ...
                        <form id="registration-form" action="{{ route('events.public_register', $event) }}" method="POST" class="space-y-5">
                            ...
                            @foreach($normalizedFields as $field)
                                @php
                                    ...
                                    $label = $field['label'];
                                    $oldValue = old("custom_fields.{$fieldId}");
                                @endphp
                                <div class="space-y-2">
                                    <label for="custom_field_{{ $fieldId }}" class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest font-semibold">
                                    ...
                                    <div class="relative">
                                        @if($fieldId === 'birthday' || strtolower($label) === 'birth date' || strtolower($label) === 'birthday' || strtolower($label) === 'birthdate')
                                            <input type="date" name="custom_fields[{{ $fieldId }}]" id="custom_field_{{ $fieldId }}" {{ $isRequired ? 'required' : '' }} value="{{ $oldValue }}" 
    ...
    <script>
        ...
        function toggleCalendarDropdown(e) {
            e.stopPropagation();
            const menu = document.getElementById('calendar-dropdown-menu');
            if (menu) {
        ...
        function triggerAddToCalendar(provider) {
            const eventData = {
                title: @json($event->title),
                description: @json(strip_tags($event->description)),
                location: @json($event->location),
                date: "{{ $event->event_date->format('Ymd\THis') }}",
                endDate: "{{ $event->event_date->copy()->addHours(2)->format('Ymd\THis') }}"
            };
            
            if (provider === 'google') {
                const url = `https://calendar.google.com/calendar/render?action=TEMPLATE` +
                    `&text=${encodeURIComponent(eventData.title)}` +
                    `&dates=${eventData.date}/${eventData.endDate}` +
                ...
                link.setAttribute('download', `${eventData.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.ics`);
