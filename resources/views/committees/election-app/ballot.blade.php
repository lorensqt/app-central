...
@section('content')
<div class="space-y-8 max-w-4xl mx-auto pb-32">
    <!-- Header panel -->
    <div class="relative overflow-hidden bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200/80 dark:border-slate-800/80 p-8 sm:p-10 shadow-sm text-center">
        ...
        <div class="relative space-y-3">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xl mx-auto leading-relaxed">
                Choose your candidates below. The ballot limits your selections to the authorized vacancies for each position.
            </p>
        </div>
    </div>

    <!-- Ballot Positions Workspace -->
    <form id="ballot-form" onsubmit="submitBallot(event)" class="space-y-8">
        @csrf
        
        @foreach($positions as $position)
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200/60 dark:border-slate-800/80 p-6 sm:p-8 shadow-sm space-y-6">
                <!-- Position Title & Max Selections Instruction -->
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800/60">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900 dark:text-white">
                            {{ $position->name }}
                        </h2>
                        <p class="text-xs text-slate-400 mt-0.5">
                            Please select your choice of candidates for this leadership role.
                        </p>
                    </div>
                    
                    <span class="inline-flex items-center shrink-0 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-purple-500/10 text-purple-600 dark:text-purple-400 self-start sm:self-center">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @forelse($position->candidates as $candidate)
                        <div onclick="toggleCandidate({{ $position->id }}, {{ $candidate->id }}, {{ $position->max_votes }}, this)"
                            class="candidate-card-btn relative bg-slate-50/50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800/80 rounded-2xl p-5 flex flex-col items-center text-center cursor-pointer hover:shadow-sm transition-all duration-200 select-none group border-transparent"
                            data-position-id="{{ $position->id }}" data-candidate-id="{{ $candidate->id }}">
                            
                            <!-- Selection Check Badge -->
                            <span class="check-badge absolute top-3.5 right-3.5 inline-flex items-center justify-center w-5.5 h-5.5 rounded-full bg-slate-200/50 dark:bg-slate-800/60 text-slate-400 group-hover:text-slate-500 transition-colors">
                            ...
                            @if($candidate->avatar_path)
                                <img src="{{ $candidate->avatar_path }}" alt="{{ $candidate->name }}" class="w-16 h-16 rounded-full object-cover mb-4 border border-slate-200/60 dark:border-slate-850 shadow-sm shrink-0">
                            @else
                                <div class="w-16 h-16 rounded-full bg-purple-50 dark:bg-purple-950 text-purple-600 dark:text-purple-400 font-extrabold flex items-center justify-center mb-4 shrink-0 border border-purple-100/30 text-lg uppercase shadow-sm">
                                    {{ substr($candidate->name, 0, 2) }}
                                </div>
                            @endif

                            <!-- Name & Party -->
                            <h3 class="font-bold text-sm text-slate-900 dark:text-white truncate max-w-full">
                                {{ $candidate->name }}
                            </h3>
                            <p class="text-[11px] text-slate-400 mt-1 truncate max-w-full font-medium">
    ...
    <div class="fixed bottom-6 left-4 right-4 md:left-auto md:right-auto md:w-full md:max-w-4xl z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-2xl border border-slate-200/80 dark:border-slate-800/80 p-5 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-4 transition-all duration-300">
        ...
        <button type="button" onclick="document.getElementById('ballot-form').requestSubmit()" id="cast-vote-btn"
            class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 active:scale-[0.98] text-white font-semibold text-xs px-6 py-3 rounded-xl shadow-md transition">
            Cast Secure Ballot
...
<script>
    ...
    function toggleCandidate(positionId, candidateId, maxVotes, cardElement) {
        ...
        if (set.has(candidateId)) {
            ...
            checkBadge.className = 'check-badge absolute top-3.5 right-3.5 inline-flex items-center justify-center w-5.5 h-5.5 rounded-full bg-slate-200/50 dark:bg-slate-800/60 text-slate-400 group-hover:text-slate-500 transition-colors';
        } else {
            // Check limits
            if (set.size >= maxVotes) {
                if (maxVotes === 1) {
                    ...
                    if (prevCard) {
                        ...
                        prevBadge.className = 'check-badge absolute top-3.5 right-3.5 inline-flex items-center justify-center w-5.5 h-5.5 rounded-full bg-slate-200/50 dark:bg-slate-800/60 text-slate-400 group-hover:text-slate-500 transition-colors';
                    }
                } else {
            ...
            checkBadge.className = 'check-badge absolute top-3.5 right-3.5 inline-flex items-center justify-center w-5.5 h-5.5 rounded-full bg-purple-600 text-white shadow-sm transition-all duration-200';
        }
