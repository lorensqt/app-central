...
<div id="edit-candidate-modal"
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 max-w-lg w-full shadow-2xl flex flex-col max-h-[92vh] transition-all duration-300 transform scale-95 opacity-0 overflow-hidden"
        ...
        <form id="edit-candidate-form" onsubmit="submitEditCandidate(event)" class="flex-grow flex flex-col min-h-0">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 sm:p-8 space-y-4 bg-slate-50/40 dark:bg-slate-950/20 flex-grow overflow-y-auto custom-scrollbar">
                ...
                <!-- Avatar URL -->
                <div class="space-y-1.5">
                    <label for="edit_cand_avatar" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Avatar / Portrait URL (Optional)</label>
                    <div class="relative rounded-xl shadow-sm">
                        ...
                        <input type="url" name="avatar_path" id="edit_cand_avatar"
