<!-- PREMIUM BACKDROP MODAL: EDIT POSITION -->
<div id="edit-position-modal"
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 max-w-lg w-full shadow-2xl flex flex-col max-h-[92vh] transition-all duration-300 transform scale-95 opacity-0 overflow-hidden"
        id="edit-position-modal-content">
            ...
            <div class="relative flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                        Update position credentials and sorting.
                    </p>
                </div>
        ...
        <form id="edit-position-form" onsubmit="submitEditPosition(event)" class="flex-grow flex flex-col min-h-0">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 sm:p-8 space-y-4 bg-slate-50/40 dark:bg-slate-950/20 flex-grow overflow-y-auto custom-scrollbar">
                ...
                <div class="space-y-1.5">
                    <label for="edit_pos_name" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Position Name</label>
                    <input type="text" name="name" id="edit_pos_name" required
                ...
                <!-- Max Votes -->
                <div class="space-y-1.5">
                    <label for="edit_pos_max_votes" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Max Selections (Voter Limit)</label>
                    <input type="number" name="max_votes" id="edit_pos_max_votes" required min="1"
                        class="w-full rounded-xl border border-slate-200 dark:border-slate-800/80 py-3 px-4 text-slate-800 dark:text-slate-200 text-sm focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 focus:outline-none bg-white dark:bg-slate-950 shadow-sm transition">
                ...
                <!-- Sort Order -->
                <div class="space-y-1.5">
                    <label for="edit_pos_sort_order" class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Display Order</label>
                    <input type="number" name="sort_order" id="edit_pos_sort_order" required
                        class="w-full rounded-xl border border-slate-200 dark:border-slate-800/80 py-3 px-4 text-slate-800 dark:text-slate-200 text-sm focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 focus:outline-none bg-white dark:bg-slate-950 shadow-sm transition">
                </div>
            </div>
